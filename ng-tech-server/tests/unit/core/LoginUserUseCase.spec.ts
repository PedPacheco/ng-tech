...
import {LoginUserResponseMock, LoginUserRequestMock} from '../../mocks/LoginUsuarioMock'
...
describe('LoginUsuarioUseCase', () => {
    ...
    it(`Quando for feita a chamada do useCase
        Então o gateway deve ser chamado corretamente`, async () => {
            const spyGateway = jest.spyOn(gateway, 'execute').mockResolvedValue({user: LoginUserResponseMock, token: 'token'})
