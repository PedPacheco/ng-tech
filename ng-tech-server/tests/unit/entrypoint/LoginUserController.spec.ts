...
import {LoginUserRequestMock, LoginUserResponseMock} from '../../mocks/LoginUsuarioMock'
...
describe('LoginUsuarioController', () => {
    ...
    it(`Quando a chamada do controller for feita
        Então useCase deve ser chamado corretamente`, async () => {
            const spyUseCase = jest.spyOn(useCase, 'execute').mockResolvedValue({user: LoginUserResponseMock, token: 'token'})
